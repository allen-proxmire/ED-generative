...
| **Novel falsifiable predictions** | ✅ ranked weapons + one confirmed forward prediction (the degenerate-mobility law, §15); the flagship distinctive bet (`a₀(z) = cH(z)/(2π)`) has met first data — evolution seen, constant `a₀` excluded decisively (ED's call confirmed, MOND's killed), exact rate mildly disfavored (~4σ nominal, one survey); ⚠️ no argument-ending weapon fully confirmed | several sharp falsifiers trace to the arrow (`a₀(z)`, w=−1, the clean-vector prohibitions) | Falsifier frontier | §15 |
