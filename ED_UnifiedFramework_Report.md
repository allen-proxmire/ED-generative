...
| **Internal consistency (anomalies)** | ✅ conservation + clean-vector baseline solid; ⚠️ chiral cancellation inherited; one candidate gated on the substrate-to-Dirac reduction | clean-vector baseline (arrow) + the worldline-arrow spectral flow | Grounded baseline / **gated** (#2) | §12 |
| **Novel falsifiable predictions** | ✅ ranked weapons + one confirmed forward prediction (the degenerate-mobility law, §14); the flagship distinctive bet (`a₀(z) = cH(z)/(2π)`) has met first data — evolution seen, constant `a₀` excluded ~30σ (ED's call confirmed, MOND's killed), exact rate mildly disfavored; ⚠️ no argument-ending weapon fully confirmed | several sharp falsifiers trace to the arrow (`a₀(z)`, w=−1, the clean-vector prohibitions) | Falsifier frontier | §14 |
...
- **⚠️** — a genuine open. There are exactly two structural ones: the gauge multiplicities #1 (§7) and the chiral anomaly cancellation #2 (§12), which converge on one wall, the unbuilt channel-topology program, plus one conjectured route (the linking argument for #1); the substrate-to-Dirac reduction they route through is itself form-complete (§10). Additionally, the state of the predictions: no argument-ending weapon is fully confirmed yet, though the flagship `a₀(z)` bet has a live partial confrontation (§14).
...
- **The MOND scale evolves with redshift, `a₀(z) = cH(z)/(2π)`.** Because `a₀` is tied to the cosmic horizon `R_H = c/H`, it tracks the instantaneous Hubble rate rather than being a constant of nature, so it grows with redshift (×1.8 by `z = 1`). This is dimensionally forced (`a₀ = c·H·number`, ED's only available scales), so ED cannot fit `a₀` the way MOND does. It is the sector's most distinctive live weapon: first data (MUSE-DARK III, 2026) confirm `a₀` evolves and exclude a constant `a₀` at ~30σ, matching ED's local value to ~8%, while mildly disfavoring ED's exact rate. This is the flagship of §14.
...
**#2, the chiral anomaly cancellation.** Whether ED's exact Gauss law forces anomaly-freedom on the emergent chiral content, ED's one possible non-inherited contribution to the deepest consistency condition in the Standard Model (§12). It is gated on the substrate-to-Dirac worldline reduction (T4). But that reduction is itself now *form-complete*: the Dirac operator's form is forced, its substrate continuum limit is computed, and the arrow's undoubling of the lattice spinor is verified (§10). What remains open is the reduction's *graph-native-spinor* residual, building the 4-spinor from the participation graph's own channel degrees of freedom, and that residual is blocked at the *same* wall as #1, the unbuilt channel-topology program. So the two structural opens are not two separate arcs but essentially *one wall*, the channel-topology program, plus the rep-spectrum's conjectured linking route to it. (Whether #2's anomaly face gates on exactly that wall or on a sibling residual of the same reduction is itself open; either way the two opens converge rather than diverge.) Two well-located problems on one wall, not a diffuse fog.
...
- **Structural opens (2):** the rep-spectrum (#1, a conjectured linking route) and the anomaly constraint (#2, gated on the substrate-to-Dirac reduction's graph-native-spinor residual). Both bounded, both unsolved in the Standard Model, and both now converging on *one* wall, the channel-topology program (the substrate-to-Dirac reduction itself being form-complete, §10).
...
**⭐ The flagship: the redshift evolution of the MOND scale, `a₀(z) = cH(z)/(2π)`.** Because ED ties `a₀` to the cosmic horizon `R_H = c/H`, the MOND acceleration scale is not a constant of nature (as in MOND) but tracks the instantaneous Hubble rate, so it must grow with redshift, roughly ×1.8 by `z = 1` and ×3 by `z = 2`. This is dimensionally forced and unfudgeable: `a₀ = c·H·(pure number)` is the only acceleration ED can build from its sole scales, `c` and the horizon rate `H`, with no second scale available to bend the power (the no-intrinsic-scalar result, §11). MOND treats `a₀` as fixed; ED structurally cannot. And the first data exist. MUSE-DARK III (2026) measures the radial-acceleration-relation scale across `0.33 < z < 1.44` and finds it evolves at ~30σ, which excludes a constant `a₀` outright, kills the MOND picture ED contradicts, and matches ED's local value to ~8%. One tension remains, on the rate: the observed evolution runs somewhat faster than `H(z)` (an implied power `α ≈ 1.2` against ED's forced `α = 1`), a mild-to-moderate disagreement from a single first-generation survey carrying large modeling systematics. So ED's qualitative call is confirmed and MOND's excluded, while ED's exact rate is disfavored but not refuted. The decisive test is a direct, multi-survey `a₀ ∝ H(z)^α` fit; `α = 0` is already dead, and ED lives or dies on `α = 1`. This is the most distinctive, most live bet in the document.
...
4. **The error-correction plateau.** ED predicts logical error floors that do not go to zero (`Γ_plateau > 0`), where standard fault tolerance predicts error → 0 with redundancy. Two sharpenings since the inventory was ranked. First, the plateau's budget is not a free-standing postulate: it is the *same* finite V5 cross-chain budget that sets the entanglement-monogamy bound and the black-hole Page-curve budget, one bounded envelope seen in three arenas, with the three tied in fixed ratio (`1 : 1 : 0.88`, the last anchored to the Bekenstein–Hawking area law). Second, and correcting the "free test as industry climbs code distance" framing: the mechanism binds on *broadcast-type* redundancy (repetition codes, cat states), not on topological surface-code distance, where the per-site budget load stays bounded and no plateau is predicted (so ED expects the clean exponential suppression the leading surface-code demonstrations show, rather than being challenged by it). The live signature is therefore a *pair* on public data: the persistence of the repetition-code error floor (already visible as an apparent `~10⁻¹⁰` floor in long repetition-code runs, attribution still contested), and a ceiling on certifiable cat-state width, both drawing on the one budget. The distinctive claim is the shared budget and its ratios; the near-term test is whether the broadcast floor survives mitigation at cross-platform-consistent content.
...
The plain status: **no distinctive, argument-ending prediction is confirmed.** One of them now has distinctive data in hand, and it is worth stating exactly what that partial result is. The flagship, `a₀`'s redshift evolution, has met its first survey: ED's qualitative call is confirmed, `a₀` evolves and a constant `a₀` (MOND's) is excluded at ~30σ, with ED's local value matched to ~8%; but ED's *exact* rate (`α = 1`) is mildly disfavored by that one survey, which prefers a somewhat faster evolution. That is a genuine, live, distinctive confrontation, not a confirmation, and not a refutation. The other bets, the envelope ratio, the cluster knee, the Class-A wall, the BTFR normalization's forward edge, remain unconfirmed. And the recent foundational work is clear about its own limits here: the quantum-logic reconstruction (§4) and the chiral-gauge sector (§9) closed structure and located walls, but they produced *no new near-term weapon*. So the highest-leverage move toward the goal is not further derivation; it is pressing the bets above, the `a₀`-evolution fit first.
...
Two structural questions are genuinely unbuilt. Why the gauge multiplicities are {1,2,3} (#1, §7) and whether ED's exact charge conservation forces anomaly-freedom on the emergent chiral content (#2, §12). These are not unexamined, and they are more unified than they first read: the substrate-to-Dirac worldline reduction, which #2 gates on, is itself form-complete (operator form forced, continuum limit computed, undoubling verified, §10), so what actually remains is its graph-native-spinor residual, which is the *same* channel-topology wall that walls #1. So the two opens reduce to **one wall and one conjectured route to it** — the unbuilt channel-topology program, and the linking argument that would, if its ED-side premise holds, tie the internal dimension to three. Both are unsolved in the Standard Model too, which postulates what ED is trying to derive.
