...
What "form-complete" means here is concrete: in each box, the substrate fixes the *form* of a law while the universe supplies its *numbers*. Gravity is the cleanest case. ED's gravity is khronometric — the arrow made dynamical — and it reproduces the weak-field Einstein metric and the classical tests, with `c_T = c` and `α₂ = 0` holding exactly, while the same khronon field accounts for dark matter (the MOND deep field) and dark energy (a single Λ); its lone scale is inherited through `H₀`, exactly as the Standard Model inherits its parameters. The scorecard in §2 records the same verdict box by box, at an explicit tier for each — quantum mechanics reconstructed, charge native, chirality a theorem, mass from binding — with the section that earns each row.

This report is as forthright about its limits as its results. Two structural questions are genuinely open — the gauge multiplicities `{1,2,3}` and whether exact charge conservation forces anomaly-freedom on the emergent chiral content — and they reduce to one deep arc (the substrate-to-Dirac reduction) plus one conjectured route; both are unsolved in the Standard Model too. One wall is *proven*: the finite-memory substrate cannot reach the primality escape layer. The constants are inherited by design. And, decisively, **no distinctive, argument-ending prediction is confirmed yet** — ED makes one confirmed forward prediction and survives the tests that killed rival gravity theories (`c_GW = c`), but the bets that would single it out remain open (§14). One primitive recurring as the mechanism across these sectors — most strikingly at the black hole, where the arrow's irreversibility preserves information as an un-erasable record — is a serious case for a unified framework; it is not yet the empirical case that would end the argument.
...
| **Novel falsifiable predictions** | ✅ ranked weapons + one confirmed forward prediction (the degenerate-mobility law, §14); ⚠️ no argument-ending weapon confirmed | several sharpest weapons trace to the arrow (α₂=0, w=−1, prohibitions) | Falsifier frontier | §14 |
...
- **⚠️** — a genuine open. There are exactly two structural ones: the gauge multiplicities #1 (§7) and the chiral anomaly cancellation #2 (§12), which reduce to one deep arc (substrate-to-Dirac) plus one conjectured route (the linking argument for #1). Additionally, the state of the predictions: none of the argument-ending weapons is confirmed yet (§14).
...
- **`α₂ = 0` exactly** — a hard preferred-frame prediction.
- **Weak-lensing depends on substrate activity** (Paper_038.6), and the **merging-cluster offset–velocity "knee"** is experiment-ready (Paper_OffsetVelocityLaw). These are on the falsifier frontier (§14).

## The debts

It is a closed sector, but it still has edges:

- **Clusters and the CMB are owed.** KM-II carries a clustering regulator dial that reproduces the phenomenology but is not yet pinned from first principles; the cluster-scale and CMB fits are the standing open debt of the line.
...
None of these demote the sector. The GR quartet is closed and Derived; the one-field unification of dark matter and dark energy is grounded and distinctive; the departures from GR are sharp enough to kill the theory if nature disagrees.
...
The Dirac operator's *form* is forced: it is the unique first-order Clifford-linear factorization of the substrate's Klein–Gordon operator, `D = iγ^μ∂_μ − mc/ℏ`. Its *substrate continuum limit* is now computed, not asserted: retarded transport on the participation graph gives `D(p) = Σ γ^μ(e^{ip_μ} − 1) ≈ iγ^μp_μ` near `p = 0` (`Paper_106`). And the **arrow undoubles it**: a naive Hermitian lattice Dirac operator has sixteen doublers, and the arrow's retardation is exactly the Wilson term that removes them, leaving a single species at the origin, verified directly (`chiral_3p1d.py`). This is ED's concrete escape from Nielsen–Ninomiya, a relational graph plus a genuine (non-Hermitian) arrow.
...
These are ED's sharpest falsifiable bets, in the corpus's own priority order.

1. **The participation-envelope ratio.** ED predicts a universal ratio `f_v ≈ 1.1·γ_dec` (the `Q/π` form) between a slow coherence-envelope modulation and the decoherence rate, holding across more than ten orders of magnitude in decoherence rate. No standard theory predicts a universal ratio spanning optomechanics to biological FRAP; decoherence is supposed to be monotonic with no slow envelope. The detector is built and validated; the accessible band is ~11–110 Hz (a FRAP Lomb–Scargle measurement). Commissioned FRAP is currently unfunded, so the live zero-cost paths are a literature-data dig or the spatial-PDE harmonic legs.

2. **The merging-cluster knee.** ED predicts a sharp, step-like knee in the offset–velocity relation of merging clusters (flat → knee → linear → ceiling), with fast mergers showing more than two lensing peaks. ΛCDM predicts velocity-*independent* scatter (no knee); MOND predicts a *smooth* roll-off (no knee). The ceiling and the >2-peak leg are testable now on El Gordo-class mergers; the full knee awaits Rubin/Euclid. The shape is robust; the knee's location is inherited.
...
4. **The error-correction plateau.** ED predicts logical error floors that do not go to zero (`Γ_plateau > 0`) and a specific exponential rigidity in the approach. Standard fault tolerance predicts error → 0 exponentially with code distance. Industry is climbing code distance now, so this is a distinctive test arriving essentially for free.

5. **BTFR zero intrinsic scatter.** ED predicts *exactly zero* intrinsic scatter in the deep-MOND asymptote of the baryonic Tully–Fisher relation. ΛCDM halo models predict nonzero scatter from formation-history variance. This is, in the corpus's words, the cleanest single number to miss, and it is consistent with SPARC data so far.
...
The plain status: **none of these distinctive, argument-ending tests have been conducted; none of these predictions have been confirmed.** The envelope ratio, the cluster knee, the Class-A wall, and the BTFR zero-scatter are the bets that would single ED out, and all are unconfirmed. And the recent foundational work is clear about its own limits here: the quantum-logic reconstruction (§4) and the chiral-gauge sector (§9) closed structure and located walls, but they produced *no new near-term weapon*. So the highest-leverage move toward the goal is not further derivation; it is executing the Tier-1 bets above.
...
Two structural questions are genuinely unbuilt. Why the gauge multiplicities are {1,2,3} (#1, §7) and whether ED's exact charge conservation forces anomaly-freedom on the emergent chiral content (#2, §12). These are not unexamined; they reduce to **one deep arc and one conjectured route** — the substrate-to-Dirac worldline reduction (which also gates the relativistic chirality statement and the graph-native spinor), and the linking argument that would, if its ED-side premise holds, tie the internal dimension to three. Both are unsolved in the Standard Model too, which postulates what ED is trying to derive.
...
Most importantly: **no distinctive, argument-ending prediction is confirmed** (§14). ED makes one confirmed forward prediction (the degenerate-mobility law) and survives the tests that killed rival theories (`c_GW = c`), but the bets that would single it out — the universal envelope ratio, the step-like cluster knee, the architecture-independent decoherence wall, the exactly-zero BTFR scatter — are all still open. Until one of them confirms, ED is a coherent, falsifiable unified framework, not a certified one. That distinction is the ceiling of this entire document.
...
# Appendix — The certified simulations
