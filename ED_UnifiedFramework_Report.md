<!-- Published snapshot. Source of truth: event-density/docs/Unification_Report/ (per-section files S00..S16 + build/). Regenerated by build/assemble_report.py; do not hand-edit this copy. PDF: event-density/docs/Unification_Report/ED_UnifiedFramework_Report.pdf -->
...
Event Density (ED) is a substrate ontology whose unifying power comes from one primitive, the **arrow of time** — the irreversible act of commitment — doing load-bearing work across sectors that mainstream physics treats as separate and even as in tension. The same arrow selects the quantum pointer basis (einselection) and *is* gravity's preferred time (the khronon), so quantum measurement's "which basis is definite" and gravity's "which time is flowing" are one primitive wearing two hats, and the problem of time dissolves rather than being reconciled. On that spine, ED is **form-complete across every box a unified framework must check** and **value-inherited exactly where the Standard Model's free parameters live** — a structural consequence of a pure-relation ontology having no intrinsic scalar, not a gap, and no worse than the Standard Model's own standing on those numbers.

The report states, box by box and at explicit honesty tiers, what is earned. Quantum mechanics is **reconstructed** from commitment by mapping onto Piron–Solèr (orthogonality reduced to operational distinguishability, the complex field selected, Born non-contextuality grounded; the residual is the Solèr lattice rigor of an inherited theorem). Gravity is **closed** — the weak-field Einstein metric, the khronometric class with `c_T = c`, `α₂ = 0` exactly, and the same khronon field giving dark matter (MOND deep field) and dark energy (one Λ), with a new healthy-branch bound `λ < 1/3, λ ≠ 1` placing ED off general relativity. The gauge sector's **shape is derived** — `SU(N)` form, non-abelian gauging, the `F²` action, the mass-gap mechanism, and a single hypercharge (the arrow wearing a phase) — the structure the Standard Model postulates wholesale. Charge is **native** (quantized winding, an integral Gauss law), with the smooth electromagnetic field a coarse-grained shadow. Chirality is a **theorem**: the parity-clean substrate is vector for every channel-count, so parity violation is necessarily spontaneous, while *which* force is chiral is inherited representation theory. And the matter sector delivers a **native binding-mass mechanism** — a lone excitation is massless, mass comes from binding, "mass without mass," the dominant form of real mass — cleanly separated from time dilation.

The report is as loud about its limits as its results. Two structural questions are genuinely open — the gauge multiplicities `{1,2,3}` and whether exact charge conservation forces anomaly-freedom on the emergent chiral content — and they reduce to one deep arc (the substrate-to-Dirac reduction) plus one delegatable computation (a linkless-embedding test of the internal dimension), both unsolved in the Standard Model too. One wall is *proven*: the finite-memory substrate cannot reach the primality escape layer, theorem-anchored. The constants are inherited by design. And, decisively, **no distinctive, argument-ending prediction is confirmed yet**: ED makes one confirmed forward prediction and survives the tests that killed rival gravity theories, but the bets that would single it out — a universal decoherence-envelope ratio, a step-like merging-cluster knee, an architecture-independent mass wall, an exactly-zero baryonic Tully–Fisher scatter — are all still open.

The claim, stated at its honest ceiling, is this: one primitive, the arrow of time, recurs as the mechanism across quantum mechanics, gravity, and the Standard Model's structure, most strikingly where quantum mechanics and gravity were supposed to be irreconcilable. That recurrence is a serious case for a unified framework. It is not yet the empirical case that would end the argument, and this document carries both openly — which is what makes it verification-grade rather than dismissible.
...
Event Density is a substrate ontology — commitment and participation, with the arrow of time as the single process primitive — advanced here as a **unified framework for physics**. The claim is not that ED reproduces one sector well. It is that one primitive does load-bearing work across sectors that mainstream physics treats as separate: quantum measurement, gravity, gauge structure, charge, chirality, and mass. The report's job is to state, box by box and at an explicit tier, how much of that is earned and how much is open.

Two words on what this is *not*. It is not a claim to have solved physics — the report carries two genuine structural opens, one proven wall, and no confirmed distinctive prediction, and it says so in every relevant section. And the framing avoids the labels "theory of everything" and "grand unified theory": the first overclaims, the second technically excludes gravity, and both trigger dismissal for reasons unrelated to content. "A unified framework for physics" is the accurate description and the ceiling of the claim.

## The boxes a unified framework must check

A framework earns the word "unified" by accounting for a specific list, and the report is organized around it:
...
- charge quantization and electromagnetism (§8);
...
A framework that checks these boxes with derivations, and states honestly where it inherits or stops, is a unified framework in the sense this report defends.

## The bar

The evidential standard is deliberately high, and the report's credibility depends on holding it. Extraordinary claims require extraordinary evidence, and for a unified-framework claim from outside the mainstream, the operative form of that standard is this: **the walls must be as loud as the results.** A report that derived the spine and buried the two structural opens would be dismissible on sight. One that states each result at its true tier, names each open at its true tier, and marks the one thing that is proven impossible, is doing the thing that makes the claim assessable rather than rhetorical.
...
3. **What is walled or open** — a limit, either proven (the one primality wall, §13) or a genuine open derivation (#1 and #3), stated as such and not smoothed.
...
- **Measured** — a result from the certified substrate simulator (the binding-mass mechanism, §10; the charge skeleton, §8).
- **Grounded** — a primitive supplies the operational content, conditionally or structurally.
- **Inherited** — a value or structure taken from measurement or standard mathematics by ED's own logic (the constants; Cl(3,1); Piron–Solèr).
- **Wall** — a proven or honest limit, the order ED does not reach (primality is the one *proven* wall).
...
| **Gravity (GR + dark matter + dark energy)** | ✅ closed (GR-I..IV + khronon MOND/DE); +healthy-branch bound λ<1/3, λ≠1 | the khronon *is* the arrow made dynamical (preferred time) | Derived (values inherited) | §5 |
| **QM + gravity unified** | ✅ distinctive — the same arrow is pointer basis *and* preferred time; the problem of time dissolves | one arrow, two demands | **Account (distinctive)** | §6 |
...
| **Matter content (spinor, mass)** | ✅ substrate→Dirac form-complete; native matter/antimatter (C); native binding-mass ("mass without mass"); mass ≠ time dilation; ⚠️ generations/Higgs inherited | the arrow undoubles the spinor + selects matter over antimatter (C) + makes the lone front massless + sets the clock | Form-complete + measured (mass) | §10 |
| **The constants (α, mass ratios, Λ)** | 📏 inherited by design — no intrinsic scalar (A1); the SM inherits them too | — (the value layer the arrow does not set) | Inherited-by-design | §11 |
| **Internal consistency (anomalies)** | ✅ conservation + clean-vector baseline solid; ⚠️ chiral cancellation inherited; one candidate T4-gated | clean-vector baseline (arrow) + the worldline-arrow spectral flow | Solid baseline / **gated** (#3) | §12 |
| **Novel falsifiable predictions** | ✅ ranked weapons + one confirmed forward prediction (UDM law); ⚠️ no argument-ending weapon confirmed | several sharpest weapons trace to the arrow (α₂=0, w=−1, prohibitions) | Falsifier frontier | §14 |

## How to read the three marks

- **✅** — ED delivers this, at the tier in the tier column. A parenthetical residual that is inherited standard mathematics (Piron–Solèr) or a rigor-completion does not demote the check; the honesty rides in the tier word (e.g. "reconstructed," not "proven").
...
- **⚠️** — a genuine open. There are exactly two structural ones: the gauge multiplicities #1 (§7) and the chiral anomaly cancellation #3 (§12), which reduce to one deep arc (substrate→Dirac) plus one delegatable tooling task. Plus the honest state of the predictions: none of the argument-ending weapons is confirmed yet (§14).

The scorecard is the skeleton of the report; the arrow column is its thesis; the tier column is its conscience. The sections deliver each row.
...
It is tempting to say "the arrow is the only primitive," and Allen has arrived at that intuition independently more than once. The honest version is sharper and survives scrutiny. When you audit ED's primitives one by one against commitment, they sort cleanly into three piles [→ reduction audit]:
...
So the precise claim is not that commitment is the only thing. It is that **commitment is the only *process*.** It is the one source of anything happening at all. The nouns and the stage are presupposed; the verb is singular. That is a weaker statement than "one primitive," and it is the one that turns out to be true, and it is still remarkable, because a single source of process is exactly what you would need for a unified theory. If everything that *happens* traces to one act, then the sectors of physics are not independent. They are that one act, wearing different clothes.

## The jobs — one arrow, sector after sector

What follows is the backbone of the report. Each line is a job the arrow does, and each job is a full section later. Read it as a promise the rest of the document keeps.
...
**The unification move** [→ §6]. Hold those two jobs next to each other. The pointer basis of quantum mechanics and the preferred time of gravity are not two facts. They are the same arrow. That single identity is ED's answer to quantum gravity, and it is the heart of this report: the two theories were never separate to begin with, they share a spine, and the spine is time committing.
...
Nine jobs, and they are not nine coincidences. They are the pattern you would expect if commitment really is the only process: apparently unrelated structures turning out to be faces of the one one-way act.

## Where the honesty lives

Two guardrails, stated here so the rest of the report can lean on them.

First, this is a **convergence, not a theorem.** No single derivation takes commitment and produces all nine jobs from it. What the report has is nine sectors, each worked separately, each landing back on the arrow. That is strong evidence and it is the kind of evidence unification is actually made of, one mechanism that keeps paying out where you had no right to expect it. But it is not a proof that the arrow is logically prior to everything, and this document will not pretend it is. A skeptic is right to press here, and §6 presses back honestly: the force of the claim is the recurrence, not a reduction.

Second, the arrow is **not everything.** It presupposes what it acts on and it needs a stage to act in. ED still carries a spatial arena it does not derive: three dimensions, a scale, a homogeneity, the kinds of rules the substrate can run. Those do not come from the arrow, and later sections that inherit numbers rather than deriving them (the constants, the representation spectrum) are inheriting them partly *because* the arrow is a process and not a stage, and a process cannot hand you the size of the room it runs in. Naming that limit up front is what keeps the spine load-bearing instead of mystical.
...
**The arrow's job here.** In §4 the arrow selected the quantum pointer basis. Here it does something a relativist is trained to say is forbidden: it supplies gravity with a **preferred time**. General relativity insists no such thing exists, that no foliation of spacetime is physically singled out. ED's gravity has one, and it is not smuggled in, it is the arrow of §3 made dynamical. The field that carries ED's gravity, the **khronon**, *is* the substrate's direction of commitment, promoted to a degree of freedom. Gravity, in ED, is what the arrow looks like at large scale.

This is the sector where ED is simultaneously most complete and most distinctive. It reproduces general relativity where GR is tested, and it departs from GR exactly where the arrow leaves a fingerprint.
...
- In its **cosmological** face, it is the dark-fluid and a single cosmological constant Λ (KM-II; the Λ *magnitude* is inherited, targets #10).

That is the payoff line of the whole sector, and it is worth stating without hedging: **gravity, dark matter, and dark energy are one field in ED, and that field is the arrow.** (Paper_OneField_Letter is the capstone.)

## The new bound: ED is on the healthy branch, not GR

One quantitative piece was tightened this session. The khronometric scalar's kinetic structure has the form `K_ijK^ij − λK² = shear² + (1/3 − λ)K²`. Ghost-freedom requires the low branch, `λ < 1/3`. General relativity is the special point `λ = 1`, and GR escapes the conformal-mode ghost using full four-dimensional diffeomorphism invariance. **ED cannot use that escape**, because the arrow makes the khronon (the expansion/conformal mode) genuinely propagate, breaking the 4-diffeomorphism symmetry GR leans on. So health *forces* ED onto the low branch: `λ < 1/3`, and in particular `λ ≠ 1`, ED is not GR (form-forced bound; the exact value of λ in `[0, 1/3)` is F-dependent and inherited). This is a distinctive, falsifiable structural claim: ED lives on the Hořava *healthy* branch, and its scalar sector is not general relativity's.

## Where ED departs from GR (the falsifiable edge)

Collecting the fingerprints, because these are the sector's weapons, not caveats:
...
- **`λ < 1/3`, `λ ≠ 1`** — the healthy-branch bound.
...
Stated plainly, because a closed sector still has edges:
...
- **Λ's magnitude is inherited**, not derived (the structure, sign, and scaling are grounded; the number is the `Θ_ED ≈ 10⁻¹²²` inherited value).
- **The exact λ is inherited** (the bound is derived; the point in `[0, 1/3)` is not).
...
The previous two sections each ended on the same primitive. §4: the arrow selects the quantum pointer basis. §5: the khronon, gravity's preferred time, is the arrow made dynamical. This section is the observation that those are not two facts. It is the report's central claim, and precisely because it is the most quotable line in the document, it is the one where the tiering has to be strictest.
...
The two demands are met by one primitive. Quantum mechanics wanted a preferred time to make outcomes definite; §4 showed the arrow supplies it, selecting the channel basis. Gravity, in ED, is built on a preferred foliation; §5 showed that foliation *is* the arrow, dynamical. So the preferred frame of the gravity sector and the preferred basis of the quantum sector are not two independent structures that have to be reconciled. They are one structure, wearing the clothes of two sectors. The problem of time does not get solved by reconciling GR's no-preferred-time with QM's need for one; it dissolves, because ED never had GR's version, it has a real arrow, and that arrow is doing both jobs.
...
The obvious objection is that this is wordplay, calling two things "the arrow" and declaring victory. Three things answer it, and none of them is a proof.
...
Third, it dissolves a recognized tension rather than papering one over. The problem of time is a real obstacle in mainstream quantum gravity, not a straw man. A framework that makes it *not arise* has done something, even before it has proven anything.

## The honesty guard (load-bearing)

Here is the discipline this section must hold, taken directly from the minimal-ontology paper's own warning about this exact claim.
...
A skeptic is right to press exactly here, and the honest response is not to push back with more assertion. It is to point at the coherence and name its tier: **distinctive, structural, and unconfirmed.**

## What this buys the rest of the report

§3 introduced the arrow; §4 and §5 gave its two largest jobs; this section identified them. From here, the Standard Model sectors (§7–§10) are the same primitive again, in smaller and more inherited roles, the single hypercharge it anchors, the continuum it sorts, the spinor it undoubles, the chirality it half-carries. None of those will be a unification on the scale of this one, and none needs to be. The claim of the report is not that the arrow does everything with equal force. It is that one primitive keeps turning up as the mechanism, most strikingly where quantum mechanics and gravity were supposed to be irreconcilable, and that this recurrence, honestly tiered, is the case for calling Event Density a unified framework.
...
The Standard Model has roughly twenty free parameters — the fine-structure constant, the fermion mass ratios, the mixing angles, the cosmological constant's value, the gauge couplings — and it derives none of them. ED does not derive them either. The question this section answers is whether that is a hole in ED or a prediction of it, and the answer, argued rather than asserted, is the latter.

## The structural reason

ED's determinability boundary (A1) established a sharp fact about the substrate: the only observer-independent scalar the substrate produces in isolation is **zero**. Controlled channel capacity between regions is exactly zero (that result is the content of §8's determinacy sibling and is derived, not posited). A substrate whose ontology is pure relation has no place to store an intrinsic dimensionless number. The corpus's own image for this is exact: asking the substrate for the fine-structure constant is like asking one molecule for the temperature of a gas. Temperature is real, but it is a property of an ensemble, not of a fragment, and no amount of interrogating the fragment yields it.
...
And where ED tried to derive a genuine dimensionless number, it failed, cleanly and on the record. The fine-structure constant was attempted by three independent routes (topological winding, an RG fixed point, a cross-overlap integral) and all three failed. The fermion mass ratios were attempted by six mechanisms and all six were refuted (a single real scalar field carries no topological invariant to quantize them). The cosmological constant's magnitude has no candidate mechanism at all — standard coarse-graining produces at most polynomial corrections, not the 122 orders of magnitude required. These negatives are exactly what the structural claim predicts: ED produces forms and O(1) factors, not the values.

## The honest resting place

The consistent conclusion, reached in the corpus and adopted here, is that the constants ED needs but does not fix — `c`, `ℓ_P`, `ħ`, and the cosmological `Θ_ED` — are **substrate constants**, on the same footing as the speed of light. They are numbers the universe hands the theory, not research targets with a self-imposed moratorium. The standing directive is "think, don't chase," now backed by a taxonomic reason (no ED move produces this kind of number) rather than an instinct.

Two clarifications keep this honest rather than triumphant. First, this does not explain the cosmological constant; `Θ_ED`'s magnitude is exactly as unexplained as before. Relocating it from "open target" to "substrate constant" is a bookkeeping correction, not a solution — it states plainly that ED inherits the number, rather than pretending a primitive produces it. Second, the "no intrinsic scalar" claim is a structural posture with strong support (the A1 zero-scalar result, which is derived, plus the taxonomy of successes and failures), not a from-nothing impossibility theorem. It is as firm as A1 and the failure record make it, and it is stated at that tier.

## The fair comparison

The decisive point for a skeptical reader is the same one that governs §7. The Standard Model does not derive its constants; they are its free parameters, fit to data. So ED inheriting them is not a deficiency *relative to the theory it is being compared against* — it is the identical inheritance. What ED adds on top is the structure the Standard Model leaves unexplained: the *form* of Newton's constant (`G = c³ℓ_P²/ℏ`), the *form* of the MOND scale (`a₀ = cH₀/2π`), the combinatorial *form* of the mixing-matrix phase count, the `F²` action, the single hypercharge. ED organizes the constants and derives the relations among the forms; it inherits the numbers, exactly as its ontology says it must, and exactly as the Standard Model does anyway.

## What this buys the report

The constants are the report's `📏` column, and they belong in a different category from the two structural opens. #1 (the rep-spectrum) and #3 (the chiral anomaly cancellation) are things ED *could* still derive and has not; the constants are things ED's own logic says it *cannot* and *need not* derive. Keeping that line sharp is what stops the twenty free parameters from reading as twenty holes. They are one principled inheritance, predicted by the substrate's lack of an intrinsic scalar, and no worse than the Standard Model's own standing on the same numbers.
...
Two things keep this honest. It is not a failure: ED is not in the business of generating the primes, and the integers are a ruler, not a target. And it is a strength of the report, not a weakness — a framework that can name exactly one thing it provably cannot do, and cite the theorem, is demonstrating that its other walls are drawn with the same seriousness. This wall is also the external twin of A1 (§8's determinacy sibling): the same finite-reach ceiling, measured once internally (controlled channel capacity is exactly zero) and once against a math-certified ruler.
...
**#1, the representation spectrum.** Why the stable channel-families are {1,2,3}, and behind it, why the internal amplitude dimension is three. ED's natural route (channel-family stability) was tested and refuted, and that is reported as a failure (§7). What remains is bounded: the question reduces to a single sharp one (why internal-d = 3), and its one live route turns on a standard, non-ED-specific computational task — detecting whether the participation graph is intrinsically linked, which needs an established linkless-embedding-detection algorithm, not new ED insight. It is delegatable. And it is unsolved in the Standard Model too, which postulates the multiplicities.

**#3, the chiral anomaly cancellation.** Whether ED's exact Gauss law forces anomaly-freedom on the emergent chiral content, ED's one possible non-inherited contribution to the deepest consistency condition in the Standard Model (§12). It is gated on the substrate-to-Dirac worldline reduction (T4), which is also what gates §9's relativistic chirality and §10's graph-native spinor. So the two structural opens are really *one deep arc* — the substrate-to-Dirac reduction — plus the rep-spectrum tooling task. Two well-located problems, not a diffuse fog.
...
So ED's limits sort cleanly, and the sorting is the honest picture:

- **Proven wall (1):** primality — theorem-anchored, the finite-memory ceiling. ED knows exactly one thing it provably cannot do.
- **Structural opens (2):** the rep-spectrum (#1, delegatable tooling task) and the anomaly constraint (#3, gated on substrate-to-Dirac). Both bounded, both unsolved in the Standard Model, both on the same two arcs.
...
None of these is hidden, and none is dressed as more settled than it is. That is the point of the section: the walls are drawn honestly and they are of different kinds, and the report's credibility comes from keeping those kinds distinct rather than collapsing them into a single apologetic "open."

## What this buys the report

The walls set up the two sections that close the report. §14 asks the harder question — not what ED has not yet derived, but what could prove it *wrong* — and §15 states the bottom line. The honest map here is what lets those sections be read as confident rather than defensive: a framework that has located its one proven limit, bounded its two real opens, and named its inheritances has earned the right to state plainly what it does and does not claim.


# §14 — The Falsifier Frontier: what could prove ED wrong

A framework earns the word "unified" by explaining, and it earns the word "scientific" by being killable. This section is the second test. It asks where ED diverges sharply enough from ΛCDM, standard quantum mechanics, and general relativity to be proven wrong at a measurable point, and it states the honest status of the one thing that would end the argument: **no distinctive, argument-ending prediction is confirmed yet.**

## What counts as a weapon

The corpus draws a line that this report keeps. A **weapon** is a prediction that is *novel* (standard physics does not make the claim), *sharp* (a number, a sign, or a shape, not a direction), *near-term testable*, and *distinctive* (ED and the standard account diverge at a measurable point). A **postdiction** — reproducing an already-known value — is consilience: it supports the framework but cannot end an argument, and several such postdictions are shared with ΛCDM, MOND, or GR. An internal-consistency check is not an empirical discriminator at all. Because ED inherits the value layer, no weapon here is a predicted *number* from first principles; the weapon is always the *form, sign, or shape*, and that is what can fail.

## The ranked weapons

These are ED's sharpest falsifiable bets, in the corpus's own priority order.

1. **The participation-envelope ratio.** ED predicts a universal ratio `f_v ≈ 1.1·γ_dec` (the `Q/π` form, corrected in 2026-07-10 from an earlier erroneous `8·γ_dec`) between a slow coherence-envelope modulation and the decoherence rate, holding across more than ten orders of magnitude in decoherence rate. No standard theory predicts a universal ratio spanning optomechanics to biological FRAP; decoherence is supposed to be monotonic with no slow envelope. The detector is built and validated; the accessible band is ~11–110 Hz (a FRAP Lomb–Scargle measurement). Commissioned FRAP is currently unfunded, so the live zero-cost paths are a literature-data dig or the spatial-PDE harmonic legs.
...
The plain status: **none of the distinctive, argument-ending weapons is confirmed.** The envelope ratio, the cluster knee, the Class-A wall, and the BTFR zero-scatter are the bets that would single ED out, and all are unconfirmed. And the recent foundational work is honest about its own limits here: the quantum-logic reconstruction (§4) and the chiral-gauge sector (§9) closed structure and located walls, but they produced *no new near-term weapon*. So the highest-leverage move toward the goal is not further derivation; it is executing the Tier-1 bets above.

Two distinctive claims are explicitly *not yet* weapons, and the report flags them as such rather than inflating them. Primitive einselection and the emergent multi-context structure (§4) are genuinely distinctive but have no concrete near-term observable. And the gauge-handedness/matter-sign correlation (§9) is theoretically firm but is flagged in the register as not testable with current data, so it belongs here as a distinctive-but-not-yet-testable claim, not a live weapon.

## What this buys the report

This is where the report is most honest and most exposed, and deliberately so. ED is falsifiable at many sharp, distinctive points — a universal envelope ratio, a step-like cluster knee, an architecture-independent decoherence wall, an exactly-zero BTFR scatter, a pinned dark-energy equation of state, a set of one-line matter-sector prohibitions. Its credibility as a unified framework ultimately does not rest on the derivations in §3–§13. It rests on confirming one of these, and that has not happened yet. The report's final section states what that means.
...
ED classifies the *form* of every sector a unified framework must address, and it does so from one primitive. The arrow of commitment reconstructs quantum mechanics by selecting the pointer basis and supplying measurement repeatability (§4); it closes gravity as the khronon, a genuine preferred time, unifying the weak-field Einstein metric with dark matter and dark energy in one field (§5); and, most distinctively, those two jobs are the *same* arrow, so the problem of time dissolves rather than being reconciled (§6). Around that spine, the Standard Model sectors are the same primitive in smaller roles: it quantizes and protects charge and casts electromagnetism as a coarse-grained shadow (§8), it is half of the chirality operator and forces parity violation to be spontaneous (§9), it undoubles the spinor and makes a lone excitation massless so that mass comes from binding (§10), and it anchors the single hypercharge of the derived gauge structure (§7). The values that fill in these forms — the couplings, the masses, the mixing angles, the cosmological constant — are inherited, and inherited *by principle*: a pure-relation substrate produces no intrinsic scalar, so it cannot and need not set a dimensionless number (§11), exactly as the Standard Model inherits the same twenty parameters.

That is a serious and coherent claim. One primitive, worked independently across sectors that mainstream physics treats as unrelated, keeps turning up as the mechanism, most strikingly where quantum mechanics and gravity were supposed to be irreconcilable.

## What is open

The report does not close, and it is exact about where it stops.

Two structural questions are genuinely unbuilt. Why the gauge multiplicities are {1,2,3} (#1, §7) and whether ED's exact charge conservation forces anomaly-freedom on the emergent chiral content (#3, §12). These are not a diffuse fog; they reduce to **one deep arc and one tooling task** — the substrate-to-Dirac worldline reduction (which also gates the relativistic chirality statement and the graph-native spinor), and a delegatable linkless-embedding computation that would test whether the internal dimension is three. Both are unsolved in the Standard Model too, which postulates what ED is trying to derive.
...
Most importantly: **no distinctive, argument-ending prediction is confirmed** (§14). ED makes one confirmed forward prediction (the degenerate-mobility law) and survives the tests that killed rival theories (`c_GW = c`), but the bets that would single it out — the universal envelope ratio, the step-like cluster knee, the architecture-independent decoherence wall, the exactly-zero BTFR scatter — are all still open. Until one of them confirms, ED is a coherent, falsifiable unified framework, not a certified one. That distinction is the honest ceiling of this entire document, and it is stated as plainly as it can be.
...
3. **The linkless-embedding test.** A bounded, delegatable computation that would either derive the internal dimension (un-walling #1) or confirm it is honestly inherited.

## The bottom line

ED is a unified framework in the strong sense — one primitive, the arrow of time, doing the load-bearing work across quantum mechanics, gravity, and the Standard Model's structure — that is form-complete, value-inherited by its own logic, honest about two genuine structural opens and one proven wall, and not yet confirmed at any of its distinctive falsifiers. The case it makes is that the recurrence of a single primitive across sectors is real and worth taking seriously; the case it does not yet make is the empirical one that would end the argument. This document carries both openly, because a unified-framework report that hid its opens and its unconfirmed status would be dismissible, and one that states them at their honest tier is not.
