...
| **Novel falsifiable predictions** | ✅ ranked weapons + one confirmed forward prediction (the degenerate-mobility law, §14); ⚠️ no argument-ending weapon confirmed | several sharp falsifiers trace to the arrow (w=−1, the clean-vector prohibitions) | Falsifier frontier | §14 |
...
These are ED's sharpest falsifiable bets, in the corpus's own priority order (which predates the `f_v` correction below, so read the numbering as the inventory's, not a fresh re-ranking). The most robust single weapon is the merging-cluster knee (#2): it is shape-based, so immune to the inherited-number and fitted-exponent problems that temper #1 and the mobility law.
...
Most importantly: **no distinctive, argument-ending prediction is confirmed** (§14). ED makes one confirmed forward prediction (the *shape* of the degenerate-mobility law; its exponents are fit) and survives the tests that killed rival theories (`c_GW = c`), but the bets that would single it out — the universal envelope ratio, the step-like cluster knee, the architecture-independent decoherence wall, the exactly-zero BTFR scatter — are all still open. Until one of them confirms, ED is a coherent, falsifiable unified framework, not a certified one. That distinction is the ceiling of this entire document.
