...
**Honest framing of SCBU's current status.** Paper ED-SC 4.2 explicitly acknowledges that the load-bearing derivation closing the synthesis — Route A: a substrate-derived $\ell_{V5}(H_0)$ relating V5 bandwidth to the cosmological rate — **does not currently close**. With Route A open, SCBU is *structurally suggestive* and supplies an organizing schema; it is not a closed cross-arc derivation. The six-projection picture is the framework's hypothesis about how cross-domain structure is organized, not a derived result. Closing Route A would upgrade the entire ED-SC 4.x arc from M3 (form-FORCED + value-INHERITED) to M2 (Intermediate Path C) and is identified as the highest-leverage open derivation in the program.

The program is **falsifiable**. Each result carries explicit falsification criteria. The SCBU framework predicts synchronized co-variation of the six projections under Hubble-tension-class shifts of $H_0$, but the **specific scaling exponents per projection axis are not currently derived** — SCBU as currently formalized predicts "co-variation in some sense" rather than a single sharp scaling law. The covariation prediction is therefore a family of falsifiers, not a single sharp falsifier; tightening this is part of the Route A closure. The sharpest currently-clean falsifier in the corpus is the **BTFR slope-4 with zero intrinsic scatter** (Paper 031), which is testable against galaxy-rotation-curve catalogs at present precision.
...
**Honest framing of the cross-domain claim.** Whether V5 is the *same kernel* (with parameters substrate-derived from a single substrate-cosmology coupling) or the *same rule-type* (with parameters domain-set by separate empirical matching in each arc) is the load-bearing distinction. The first interpretation requires Route A in Paper ED-SC 4.2 (substrate-derived $\ell_{V5}(H_0)$) to close; the second is what the corpus currently delivers. With Route A open, the V5 cross-domain pattern is "one rule-type appearing across many domains with currently un-derived parameter relations between them" — structurally suggestive, but weaker than "one kernel forced by substrate-cosmology coupling." This distinction is preserved consistently in §7 (SCBU) and §8 (ED-SC 4.x); the present section names the pattern, not a closed derivation of it.
...
**What SCBU claims, with honest scope.** SCBU is offered as the framework's **organizing structural hypothesis**: that the substrate-cosmology boundary at $R_H = c / H_0$ — where V5 cross-chain bandwidth saturates — is a single substrate-level object, and that multiple physical quantities at different domains are projections of this single object onto different parameter axes. The hypothesis is structurally suggestive; it is **not currently a closed cross-arc derivation**.

The reason for the caveat: Paper ED-SC 4.2 explicitly identifies the load-bearing derivation closing the synthesis — Route A: a substrate-derived $\ell_{V5}(H_0)$ relation between V5 bandwidth and the cosmological rate — as **open**. With Route A open, the six-projection picture is the framework's organizing schema, not a derived result. The V5 kernel does structurally identical *jobs* across domains (Maxwell relaxation at $\tau_M \sim 10^{-3}$ s in soft-matter, cutoff $\omega_c = c/\ell_P$ in Hawking, entanglement-bandwidth modulation, Q-Compute multiplicity cap, NS-Q dissipation, BH decoupling); whether these jobs are performed by *the same* kernel (with parameters substrate-derived by a single substrate-cosmology coupling) or by *the same rule-type* (with parameters domain-set by separate matching) is the load-bearing distinction. The first interpretation requires Route A; the second is what's currently in the corpus. SCBU is framed for the first; with Route A open, the corpus delivers the second. The honest framing in §1 reflects this.
...
**The structural claim, honestly framed:** *one substrate, one boundary, six projections offered as the organizing hypothesis; substrate-cosmology coupling (Route A) currently OPEN.* The strength of the claim grows with closure of Route A, and shrinks if the empirical covariation prediction is falsified along any single projection axis.
