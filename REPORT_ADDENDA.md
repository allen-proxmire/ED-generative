# Unification Report — Addenda & Updates

> **✅ A1–A10 APPLIED to report v2 (2026-07-15, commit `b40dd80`).** The entries below were folded into `ED_UnifiedFramework_Report.md` (v1.0 md+pdf archived to Desktop). They are retained as the audit trail of *how* v2 was reached; do **not** re-apply. New findings after v2 accumulate below the `--- post-v2 ---` marker at the end for a future v3.

The report accumulates post-release findings **here** rather than being edited in place between versions; a batch is then folded into the next version (as A1–A10 were folded into v2). This list is the raw material for the next consolidated update.
...
**Rebalance (2026-07-14) — don't over-bank this negative either.** Two prior notes here leaned too hard on the tension. Corrected weighting: (i) the ~30σ is for *evolution existing* (a₀≠const) — that KILLS MOND-constant and CONFIRMS ED's qualitative call + local value (8%); solidly ED-favorable. (ii) The *faster-than-H(z)* (anti-ED) part is a **weak preference** — few-σ statistical, ~1–2σ once systematics dominate, from **one first-generation survey** (79 galaxies, ~20/bin, z≤1.44, disk-halo + pressure-support modeling, scatter→0.19 dex, and a ~1.7× absolute-a₀ offset flagging large systematics). (iii) **ED's α=1 is dimensionally forced** — a₀ = c·H·(pure number) is the only acceleration buildable from ED's sole scales (c, horizon H); α≠1 needs a second scale ED structurally lacks (no intrinsic scalar / FSC). So α=1 is unfudgeable — a clean falsifiable bet, not a wounded fit. **Honest status: qualitative call confirmed (ED beats MOND); exact rate mildly disfavored by first data; OPEN tension awaiting Rubin/Euclid-era confirmation — not a refutation.** If better surveys robustly confirm α>1, ED cannot accommodate it (a real problem then); but one first-gen survey doesn't get there.

---
## --- post-v2 --- (new findings accumulate here for the next version)
