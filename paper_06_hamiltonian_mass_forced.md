...
The non-relativistic Hamiltonian $\hat{H} = \hat{p}^2/(2m) + V(\hat{x})$ — quadratic kinetic operator, factor of $1/(2m)$, additive potential — is the dynamical generator of standard quantum mechanics. Its specific structural form, together with the appearance of an inertial mass parameter $m$ as a Lorentz-scalar carrier of dynamical content, is normally a postulate. This paper shows that, given a small set of substrate conditions and the participation-measure, Born-rule, inner-product, and Schrödinger results of Papers #1-#4, the entire structure is forced: the kinetic-plus-potential decomposition follows from translation invariance plus locality; the quadratic dependence on $\hat{p}$ follows from rotational invariance plus the non-relativistic-limit scope; the factor of $1/(2m)$ falls out of the Galilean Lie algebra commutator $[\hat{H}, \hat{K}] = -i\hbar\hat{p}$ as the integration Jacobian; and mass appears structurally as a Lorentz-scalar bandwidth-signature functional with units of energy. Linear, higher-power, anisotropic, non-quadratic, fundamental-mass, and MOND-class alternatives are each excluded by substrate-condition violation. The numerical value of mass and the absence of forced mass ratios are inherited from the value layer.
...
> **Forcing Theorem (Hamiltonian Form + Mass Structure).** Let any substrate satisfy the conditions $\{C\}$ stated in §5. Then the self-adjoint Hamiltonian generator of Paper #4 is uniquely (up to inherited numerical values) the operator
> $$
> \hat{H} = \frac{\hbar^2\,|\hat{p}|^2}{2m} + V(\hat{x}),
> $$
> with mass $m$ appearing as a Lorentz-scalar bandwidth-signature functional of substrate primitives. The kinetic-plus-potential decomposition, the quadratic dependence on $\hat{p}$, the factor of $1/(2m)$, and the structural form of $m$ are FORCED. The numerical value of $m$, mass ratios across rule-types, mass hierarchies, and the number of generations are INHERITED.
