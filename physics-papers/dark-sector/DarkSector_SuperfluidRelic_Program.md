...
**Status: CANDIDATE RESEARCH PROGRAM, not a result.** This document consolidates a chain of reasoning (2026-07-19 session) into one honestly-tiered picture, states what is standing vs. candidate vs. open, and names the single calculation that would confirm or kill it. Nothing here is claimed as derived. Read it as a map of a promising direction with every brake marked — the "before" state a decisive calculation would test.
