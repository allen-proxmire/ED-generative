...
**Status: CANDIDATE UNIFICATION, OPEN — UNDER-TESTED (the "falsification" of 2026-07-19 was retracted the same day as a wrong-variable artifact).** The khronon-mode probe (`v5_khronon_mode_probe.py`) measured the V5 phonon's *phase-stiffness* as standard (`(∇φ)^{2.00}`) and I briefly read that as falsifying the unification. **That was premature — the probe could not test MOND, because it left out the two ingredients MOND needs:** (1) the scale **`a₀`** — MOND is *defined* by `a₀ = cH₀/2π`, which in ED comes from the *cosmic horizon* (KM-I), not the local relic gas; a scale-free setup is analytic at small gradient and *must* return `(∇φ)²` (Newtonian), so the result was guaranteed by the missing scale, not a property of ED; and (2) the **density–phase coupling** — in a superfluid the MOND structure lives in the density/chemical-potential sector, which the probe held fixed while varying only the phase. **So what stands is narrow and true (the bare V5 phase-stiffness is standard); the unification is neither confirmed nor refuted — the decisive test has not actually been run.** The correct probe (below) is harder: the relic condensate coupled to the horizon-derived `a₀`, sourced by localized baryons, with the *density* responding, force law read off. The unification sections below are the live hypothesis; the retracted-falsification note is preserved in the decisive-test section for the audit trail.
