...
**Status: THE UNIFICATION IS FALSIFIED (2026-07-19). The document is kept as the honest record of a candidate that was built, tested, and refuted on its central claim.** The decisive khronon-mode probe (`event-density/evaluation/DarkSector/v5_khronon_mode_probe.py`) found the V5-condensed collective mode has a **standard (Newtonian) kinetic term** (`ΔE ~ (∇φ)^{2.00}`), not the non-analytic MOND (AQUAL) structure (`~(∇φ)^3`). **So the condensed relic is NOT the khronon, and "one substance carries both dark matter and MOND" does not close.** What survives is the relic as a *dark-matter* candidate (condensation, mass-independence, thermal split — §"tests below"); what falls is the unification. ED reverts to its standing two-part picture: **MOND = the separate khronon (KM-I) + a relic = dark matter** — with the double-counting and mass tension the superfluid picture was meant to dissolve now returned un-resolved. Read the unification sections below as the falsified hypothesis, not a result.
...
- **(b) The khronon-mode test — RUN 2026-07-19, FAILED (`v5_khronon_mode_probe.py`).** The V5-condensed collective mode's kinetic term is `ΔE ~ (∇φ)^{2.00}` — **standard/Newtonian**, not the `(∇φ)^3` (AQUAL/`√`) structure MOND requires. The `cos`-based V5 functional is analytic (`(∇φ)², (∇φ)⁴, …`) and never produces the non-analytic `√` term. **So the condensed mode is Newtonian, NOT the khronon — the unification is falsified.**
