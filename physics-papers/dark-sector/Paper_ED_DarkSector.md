...
- **The flagship bet — the MOND scale evolves: $a_0(z) = cH(z)/(2\pi)$.** Because $a_0$ is the horizon read live, it tracks the instantaneous Hubble rate, so $a_0(z)/a_0(0) = H(z)/H_0$ (a factor ~1.8 by $z=1$). This is not a constant of nature (MOND) nor absent (ΛCDM); it is a parameter-free, dimensionally forced shape. Its first data confirm it in the decisive respect: evolution is detected at high significance, and a *constant* $a_0$ is excluded, which kills MOND's constant-scale picture. ED's exact rate ($\alpha=1$ in $a_0 \propto H^\alpha$) is mildly disfavored by that same data (a measured $\alpha$ modestly above 1), an honest tension of order one to two sigma awaiting Rubin/Euclid-era confirmation, not a refutation. This is the sharpest, most data-live bet in the sector. (Not to be confused with its companion: dark energy's Λ is the *frozen* horizon and does not evolve, and that is a separate box, the dark-energy sector, not this bet.)
