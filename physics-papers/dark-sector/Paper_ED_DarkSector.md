...
- **The flagship bet — the MOND scale evolves: $a_0(z) = cH(z)/(2\pi)$.** Because $a_0$ is the horizon read live, it tracks the instantaneous Hubble rate, so $a_0(z)/a_0(0) = H(z)/H_0$ (a factor ~1.8 by $z=1$). This is not a constant of nature (MOND) nor absent (ΛCDM); it is a parameter-free, dimensionally forced shape, because ED has no second scale from which to build $a_0$, so $a_0$ must be $c\,H\times(\text{number})$ at every epoch. It is the sharpest distinctive test in the sector, and it is cleanly decidable: measure $a_0$ against redshift in high-$z$ rotation-curve samples, where a *constant* $a_0$ vindicates MOND and refutes ED, while $a_0 \propto H(z)$ is ED's signature. The decisive high-$z$ data are **imminent rather than in hand** (deep-field spectroscopy of the MUSE class, and Rubin/Euclid-era rotation curves), which makes this a live, near-term falsification rather than a postdiction. (Not to be confused with its companion: dark energy's Λ is the *frozen* horizon and does not evolve; that is a separate box, the dark-energy sector, not this bet.)
...
- **F3.** The MOND scale shown *constant* with redshift, $a_0(z) = a_0(0)$ — refutes the flagship $a_0(z)$ bet. The decisive high-$z$ measurement is imminent.
...
| **a₀(z) = cH(z)/(2π) evolution** | **derived shape; distinctive falsifiable prediction, decisive test imminent (not in hand)** (flagship) |
