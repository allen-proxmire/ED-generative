...
- **The flagship bet — the MOND scale evolves: $a_0(z) = cH(z)/(2\pi)$.** Because $a_0$ is the horizon read live, it tracks the instantaneous Hubble rate, so $a_0(z)/a_0(0) = H(z)/H_0$ (a factor ~1.8 by $z=1$). This is not a constant of nature (MOND) nor absent (ΛCDM); it is a parameter-free, dimensionally forced shape. Its first data confirm it in the decisive respect: evolution is detected at high significance, and a *constant* $a_0$ is excluded, killing MOND's picture. ED's exact rate ($\alpha=1$ in $a_0 \propto H^\alpha$) is mildly disfavored by that first survey (a measured $\alpha$ near 1.2), an honest tension of order one to two sigma awaiting Rubin/Euclid-era confirmation. This is the sharpest, most data-live bet in the sector. (**Note the companion, not this bet:** dark energy's Λ is the *frozen* horizon and does not evolve; that is a separate box and, unlike $a_0$, currently under its own observational pressure.)
...
- **F3.** The MOND scale shown *constant* with redshift, $a_0(z) = a_0(0)$ — refutes the flagship $a_0(z)$ bet (already disfavored by the data, which see evolution).
...
| **a₀(z) = cH(z)/(2π) evolution** | **derived shape; evolution data-confirmed, exact rate in tension** (flagship) |
...
| Bullet offset via the relic (collisionless) | **standing-grade** (inherited ΛCDM mechanism); the offset-velocity knee is a **speculative bonus** |
