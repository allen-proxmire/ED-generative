...
- Substrate audit of dark-energy time-evolution — **RESOLVED as constant, $w = -1$ (2026-07-16; see Paper_ED_Cos_05).** $\Lambda$ is the frozen late-time saturation-floor value, *not* a live-horizon quantity, and this is forced by viability: a $\Lambda$ tied to the *instantaneous* Hubble radius $R_H = c/H(z)$ gives $\rho_\Lambda \propto H^2 \Rightarrow w = 0$ (no acceleration — the Hubble-cutoff holographic failure; the §3.4 $\sim M_P^2 H^2$ form is exactly this if evaluated live), so only the frozen-floor value accelerates. Contrast $a_0$ (Paper_029), which *does* ride the live horizon and so evolves as $a_0(z) = cH(z)/(2\pi)$: $a_0$ is a scale (can track live $H$ harmlessly), $\Lambda$ is a vacuum energy (cannot). **Residual OPEN:** whether the substrate *cleanly forces* the frozen-floor value or merely *stipulates* it — the saturation regime (Cos_05) supplies "frozen," but the horizon integral of this paper alone does not, and there is a bootstrap knot (acceleration is needed to reach the de Sitter state that freezes $H$; ED's actual driver of the transition is matter-dilution, Cos_05 §3.3). The falsifier F3 (below) stands: any detected $\Lambda$ evolution would now be a genuine problem, since ED is committed to $w = -1$.
