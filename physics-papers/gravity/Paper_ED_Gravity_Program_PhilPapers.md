...
**Update — companion paper GR-IV (*The Arrow's Alibi*) computes the front.** The decisive step is to recognize that the preferred-frame parameter probes only the *local* khronon stiffness, $\alpha_1 = -4\lambda_{\rm local}$, and that this stiffness is the *commitment sparsity*: $\lambda_{\rm local} \sim \rho_{\rm event}/\rho_{\rm Planck}$, because the metric stiffness comes from the always-on reciprocal sharing while the khronon stiffness comes from the sparse act of commitment. Commitment is *forced* to be sparse — dense commitment is the quantum Zeno limit, which would freeze all evolution and abolish ED's quantum mechanics — so $\lambda_{\rm local} \ll 1$ in every sub-Planck environment and $\alpha_1$ is suppressed below the bound by $\gtrsim 70$ orders of magnitude. *(derived mechanism; the magnitude is order-of-magnitude but robust by $\gtrsim 70$ orders.)* The front is therefore closed **favorably** — and it remains a live falsification *test*: a measured $\alpha_1 \gtrsim 10^{-5}$ in any sub-Planck-density regime would still kill the theory. Strikingly, the same sparseness of becoming that suppresses $\alpha_1$ is what gives ED quantum coherence and gravitational time dilation — one structure, three faces. This is, deliberately, the sharpest place the theory exposes its falsification point, and the place where, once computed, it survives.
...
- **The preferred-frame numbers $\alpha_1, \alpha_2$** (§9) — *computed in companion work (GR-IV)*: density-suppressed by the sparseness of commitment, $\alpha_1 = -4\lambda_{\rm local} \sim \rho_{\rm event}/\rho_{\rm Planck}$, safe below the bound by $\gtrsim 70$ orders. **Closed favorably**, and still a live falsification *test* (a measured $\alpha_1 \gtrsim 10^{-5}$ kills it). No longer the open front.
