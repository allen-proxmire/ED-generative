...
- **F1 — a measured preferred-frame effect.** If $\alpha_1$ (or $\alpha_2$) is measured nonzero at or above $\sim 10^{-5}$ in any environment, the density-suppression mechanism is excluded. (The prediction is that they are unobservably small in all sub-Planck-density tests.)
...
*GR-IV. Closes the preferred-frame falsification front of the ED gravity line. The PPN preferred-frame parameter reduces to the local khronon stiffness, $\alpha_1 = -4\lambda_{\rm local}$ (from $c_T=c$, $c_s=c$); the stiffness is the commitment sparsity, $\lambda_{\rm local} = (k_{11}/s_{02})\,\rho_{\rm event}/\rho_{\rm Planck}$, because the metric stiffness is always-on P02 sharing while the khronon stiffness is sparse P11 commitment; and commitment is forced sparse because dense commitment is the quantum Zeno limit (no quantum mechanics). Hence $\alpha_1 \lesssim 4\times10^{-93}$ — safe by $\gtrsim 70$ orders. MOND ($a_0 = cH_0/2\pi$, Paper 029) is the khronon's cosmological face and is untouched. Sparse becoming gives ED quantum coherence, gravitational time dilation, and preferred-frame safety — one structure, three faces. ED is a screened khronometric theory: observationally General Relativity, distinct only at the Planck-density frontier. Conditional on its primitives, falsifiable at four marked fronts; refines GR-II (form) with the coupling magnitude; no number fabricated.*
