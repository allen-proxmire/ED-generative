...
| Radiative: scalar quadrupole rate / polarization vs GR tensor | **OPEN TENSION** | §6, §10.4; the sharpened weak-field pulsar/LIGO test |
...
- **The quadrupole rate and polarization are the sharpened open tension.** A scalar quadrupole differs from GR's tensor quadrupole in rate (pulsars match GR to `~0.1%`) and polarization (LIGO favours the two tensor modes over a scalar breathing mode). Whether ED's scalar radiation matches, or emergently generates a tensor sector, is open (§10.4). So the tension is narrowed from "does it radiate / dipole catastrophe" to the quadrupole rate/polarization, a specific, still-falsifiable point.
...
- **Radiative gravity: partially resolved, the quadrupole part open (§6).** ED does radiate (the scalar bandwidth field is retarded), and it structurally evades the *dipole* catastrophe (the strongest scalar-gravity pulsar killer) because its universal bandwidth coupling `Q ∝ M` is the equivalence principle, giving a vanishing scalar dipole and quadrupole-leading radiation. What remains open is whether the scalar *quadrupole* rate and polarization match GR's tensor quadrupole (pulsar `0.1%` rate + LIGO polarization); that is the sharpened live tension, and it is weak-field, not covered by the strong-field hedge below.
...
**Falsifier sentence:** *Two live instances. (a) Radiative (sharpened): ED's scalar bandwidth radiation structurally evades dipole radiation (via the universal `Q ∝ M` equivalence-principle coupling, §6), so the pulsar/LIGO test reduces to the quadrupole; a scalar-quadrupole rate or polarization inconsistent with the measured binary-pulsar decay (`~0.1%`) or the LIGO tensor-polarization preference, with no emergent tensor sector, would falsify the kinematic-metric account. (b) Galactic: rotation curves requiring dark matter rather than the MOND interpolation, or the absence of the deep-MOND regime, would falsify the MOND-covariantization prediction. These are the sharp ED-vs-GR discriminators, and (a) is not deferrable to strong field.*
