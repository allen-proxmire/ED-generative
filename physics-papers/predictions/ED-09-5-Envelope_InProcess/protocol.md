...
> **⚠⚠ CORRECTION 2026-07-10 — the envelope FREQUENCY multiplier is wrong by ~7×.** Driving the test from the *actual* canonical PDE ([`ED_PDE_Drive_Finding_2026-07-10.md`](ED_PDE_Drive_Finding_2026-07-10.md), `scripts/ed_pde_envelope_drive.py`) shows the participation envelope sits at **`f_v ≈ (Q/π)·γ_dec ≈ 1.1·γ_dec`, NOT `8·γ_dec`.** The `N_osc=8` in §15/§2 is the ED-Phys-17 turning-point count *down to ~10⁻³ amplitude* (`N(f)=Q·ln(1/f)/π` → `N_osc=8 ⟺ Q=3.5, f=10⁻³`); §15's formula requires the cycle count per *1/e* time, which is `Q/π≈1.1`. The PDE at canon-default ζ=¼ reproduces the Canon's own `Q≈3.5` invariant and gives `f_v/γ_dec ≈ 1.1` across the oscillatory regime (nonlinearity does not rescue `8`). **The prediction survives but its band moves down ~7×: Track B → ~11–110 Hz (more accessible — needs only ≥~250 Hz framerate, not 2 kHz); Track A (Delic/Magrini) → ~2.2 mHz, ~450 s period.** The §2 table `f_v = 8·γ_dec`, the "80–800 Hz" band throughout, and the 16 mHz / 63 s optomechanics numbers are superseded by the `Q/π` multiplier. Fix before any commissioned test or Aspelmeyer ask. *(Note: this contradicts the 2026-04-22 banner's "envelope prediction is unaffected" — the frequency IS affected.)*
