...
|---|---|---|---|---|---|
| 1 | **Participation-envelope modulation** (Bundle §10 / ML 4.6) | a universal ratio `f_v ≈ (Q/π)·γ_dec ≈ 1.1·γ_dec` **[CORRECTED 2026-07-10 from the erroneous `8·γ_dec`]**, same across 10+ OOM in decoherence rate | no theory predicts a universal ratio spanning optomechanics→FRAP; decoherence is monotonic, no slow envelope | FRAP Lomb–Scargle **Track B**, band ~11–110 Hz (needs ≥~250 Hz framerate); optomechanics Track A (~2.2 mHz) | **detector built + validated; band corrected via the real PDE; commissioned FRAP unfunded ($15k)** |
...
- **Honest note on the recent foundational work:** Gate 1 (QM-foundations reconstruction) and Gate 2 (chiral gauge) closed structure and mapped walls but produced **no new near-term weapon**. The highest-leverage move toward the goal is therefore **executing the Tier-1 bets above** — not further derivation.
- **Envelope (#1) status 2026-07-10:** the Track B detector is built + validated (`ED-09-5-Envelope_InProcess/scripts/`), and **driving the test from the real canonical PDE caught a ~7× error in the predicted frequency** — the true envelope is at `≈1.1·γ_dec` (= `Q/π` with the Canon's own `Q≈3.5`), not `8·γ_dec`; the `8` was a `10⁻³`-amplitude threshold cycle-count mis-used in a `1/e`-time formula (`ED_PDE_Drive_Finding_2026-07-10.md`). The prediction survives, sharper and ~7× lower (band ~11–110 Hz, *more* accessible). Commissioned FRAP is **not being funded** ($15k quote declined, AP 2026-07-10); the live zero-cost paths are a literature-data dig or a spatial-PDE run of the harmonic/triad (F4/F5) legs.
