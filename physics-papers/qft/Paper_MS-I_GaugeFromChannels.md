...
2. **The Standard-Model gauge group $U(1)\times SU(2)\times SU(3)$ is not derived.** This paper derives that gauge groups are $SU(N)$ from channel multiplicity, and that the SM groups *correspond* to multiplicities $\{1,2,3\}$. It does **not** derive *why* the stable multiplicities stop at 3 (the uniqueness question, §7). A candidate that bounded the multiplicity by the spatial dimension is shown to be a category error and is retracted (§7.1).
3. **The weak force's specific chirality is not derived.** The paper shows parity violation is confined to the non-abelian sector and that all chirality originates in the arrow's first commitment; it does **not** derive why the *weak* $SU(2)$ in particular couples to one handedness (the matter-assignment, §7.2), which is entangled with the Standard Model's own open problems (the strong-CP problem, the CKM phase). A candidate "commitment-coupled" mechanism was tested and refuted (§7.2).
...
| Uniqueness $\{1,2,3\}$ | **OPEN** | §7.1 — spatial-dimension bound a category error, retracted |
| Weak-specific chirality (why left-handed doublets) | **OPEN** | §7.2 — commitment-coupled hook refuted |
...
### 7.1 Why $\{1,2,3\}$? — open; the spatial-dimension bound is a category error

§3 gives $SU(N)$ for any $N$; the SM stops at 3. A tempting candidate bounded the multiplicity by the spatial dimension ($N\le D_{\text{spatial}}=3$, channels being spatial directions). It is a **category error** and is retracted: three *real* spatial axes give the rotation group $SO(3)$ (real, dimension 3), whereas the gauge group is $SU(3)$ (complex, dimension 8) — the $SU(N)$-giving channels are a *complex* internal triplet $\mathbb{C}^N$, not the real spatial frame, and identifying them would make color rotation-variant (it is not). The numerical match $N_c=3=D_{\text{space}}$ is most likely a coincidence ($N_c=3$ is pinned in the SM by anomalies, the $\pi^0\to\gamma\gamma$ rate, and the $R$-ratio, none referencing spatial dimension). The multiplicity bound, if real, is **internal** — a channel-family *stability* ceiling (multiplicity is "usually small"; a coherence limit on $N$) — and its value is open. A slim, unbuilt rescue (a $3$-dimension-specific *topological* bound, since braiding/linking is special to 3D) is noted but not claimed.

### 7.2 Why the weak force specifically is chiral — open; the commitment hook is refuted

§5 confines parity violation to the non-abelian sector, but both the weak ($SU(2)$) and strong ($SU(3)$) forces are non-abelian, and the strong force is *vector*. So chirality is set by the matter **handedness-assignment** — which channel-topology classes the force couples — not by the group. A candidate that the weak force is chiral because it is the "commitment-coupled / flavor-changing" force was **tested and refuted**: the weak *neutral* current (the $Z$) is chiral but flavor-conserving, so chirality is not tied to flavor-change. A better version — the weak force as the uniquely T/CP-violating force, hence the arrow-coupled one — is a real correspondence but bottoms out on the Standard Model's own hardest problems (the strong-CP problem; the CKM-phase origin of CP violation) and does not distinguish weak from strong at the kernel level (both are V5). So the weak-specific chirality is **open**: §6 supplies the global handedness *reference*; it does not single out the weak force as the one that couples to it asymmetrically.
...
**What this paper does not claim.** It does not derive the Standard-Model gauge group (why the multiplicities stop at $\{1,2,3\}$ is open; the spatial-dimension bound was a category error); it does not derive the weak force's specific chirality (the matter-assignment is open; the commitment-coupled hook was refuted); it does not derive the Yang–Mills dynamics, the electroweak/Higgs sector, or anomaly cancellation (deferred); §6 is an account, not a closed proof, and the selection direction is a contingent initial condition (spontaneous symmetry breaking). $\kappa$, masses, mixings, and the multiplicities themselves are inherited.
