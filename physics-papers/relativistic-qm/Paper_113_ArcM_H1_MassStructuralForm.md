...
| 10 | Substrate-Higgs mechanism for vector bosons (§3.3) | OPEN | Substrate-level Higgs mechanism asserted, not derived from primitives. Scoped in more detail (not derived further) by Arc-Q Q.4 `higgs_mechanism_scoping.md`: H1 ADMISSIBLE-CLEAN, occupancy EMPIRICAL. Tested on the certified reference substrate (E1, `theory/Higgs_Emergence/E1_MassFromStructure_Results.md`): H2/σ_τ does NOT realize a mass on the reference ρ-field (extinction/channeling, not dispersion); reference substrate lacks an amplitude-invariant field for σ_τ to live on. **UPDATE 2026-07-06** (`theory/Higgs_Emergence/Dwell_Channel_Mass_Results.md`): a sixth, previously-untried candidate — a self-referential ("dwell") channel, licensed by canonical P02+P03+P07 (none of which forbid a channel from a locus to itself) — was built and run on the certified substrate. Under an asymmetric coefficient choice (kg>ks, within SigmaCoeffs' own licensed tuning range), it produces a genuine, stable, tunable, SURVIVING sub-ballistic propagation mode (v_eff down to ~0.01 of the ballistic rate, confirmed over 20,000 steps, never extinguishing) — the real dispersion-gap signature H1 and H2 both lacked. Not a Higgs/EWSB mechanism on its own (no symmetry breaking, no condensate, no gauge-boson coupling shown) and conditional on an undischarged coefficient choice, not FORCED. Row stays OPEN, but the gap is no longer a closed dead end — a concrete, tested, positive structural handle now exists where none did before. **UPDATE 2026-07-06, second finding** (`theory/Higgs_Emergence/Dwell_Intrinsic_Memory_Results.md`): the strongest positive result of this whole line. A genuinely chain-carried "history" value — travels with a chain at every commit, unlike existing `rho`/`orientation` state, and is actually read by the movement rule, unlike `orientation` — with a fading-memory update rule produces stable, bounded, SURVIVING slowing (v_eff settles ~0.65-0.72 of ballistic, confirmed over 8000 steps, 6 seeds, no external reference position anywhere). Unlike the first dwell-mass finding, this "heaviness" is intrinsic to the chain itself, not tied to a coefficient asymmetry or a fixed source location — closer to what mass actually is. Still requires new state (honestly named, not derived from P02–P13) and two free parameters (coupling strength, fade rate). Row stays OPEN; the missing-ingredient diagnosis from the first update is now also the basis of a working positive mechanism. |
