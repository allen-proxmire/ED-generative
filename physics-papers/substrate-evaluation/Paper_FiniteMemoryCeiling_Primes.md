...
- **(ii) irreducible bilinearity — OPEN, and the predicted obstruction.** The twin fluctuation has ≈0 linear correlation with the single-zero amplitudes (`corr(Δ₂, M) ≈ 0.06`, `corr(Δ₂, ψ−x) ≈ −0.03`), consistent with the twin correlation `Σ Λ(n)Λ(n+2)` being irreducibly **bilinear** (governed by *pairs* of zeros — Montgomery pair-correlation, Bogomolny–Keating), so a *single*-amplitude `|A|²` is likely the wrong shape. Not settleable at this `N`. Tracked as its own analytic fork — `OPEN_QUESTION_FS_TB_QM_AMP_02_Bilinearity.md` (Twin Bertrand repo) — which anticipates the resolution is a **two-point / pair** correlation (a covariance over pairs of zeros) rather than a single-amplitude square. Under active analysis; both outcome routes (form-level predicted / mechanism-level unlikely) are pre-registered in that file's §8 landing pads, so the resolution integrates deterministically.
