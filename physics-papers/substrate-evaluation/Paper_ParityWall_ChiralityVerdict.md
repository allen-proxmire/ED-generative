...
A wall is a legitimate, and in this case valuable, result. It is stated cleanly, it is falsifiable (§7), and it comes with a single unifying reason (ED has no handed spatial structure) and one genuine *positive* alongside it (a natively-selected matter/antimatter reference, a native selection, with an account-tier identification as C and an inherited magnitude). The paper's discipline is to distinguish sharply between the two things a "chirality" claim can mean, C (matter/antimatter, charge conjugation) and P (parity, handedness), because ED delivers the first and walls the second, and conflating them is exactly the over-read this paper corrects.
...
**One honest caveat (flagged, not resolved).** "Parity-inert" is not automatically "parity-safe": in field theory an *internal* U(1) phase can be promoted to a genuine *pseudoscalar* by a chiral anomaly (the η′, axion, θ-vacuum are exactly parity-odd phase-on-a-circle objects, made P-odd by anomalous coupling to spacetime). The clean chain "internal U(1) ⟹ inert ⟹ C not P" therefore assumes the P09 U(1) is **not anomalously P-odd in 3+1D**, an open check (and the corpus's own 1+1D winding=anomaly result, §4, is exactly where such a coupling would live). If P09's phase were anomalously parity-odd, the selection route could give P after all, and the earlier "imprints both C and P" account would be partly rehabilitated. Note this is the *same* anomaly that Route 2 (§4) invokes with the opposite sign: §4 reads the 1+1D winding=anomaly result as *support* for the wall (the arrow gives chirality only in 1+1D, where it is the spatial direction), while here the worry is that an analogous anomaly in the 3+1D P09 sector could *supply* P. So this is **one** open question, whether the P09 U(1) is anomalous in 3+1D, doing double duty, not two independent mentions; resolving it settles both the §4 support and the §5 threat. It is a real residual on Route 3, recorded here (and it touches only Route 3: even an anomalous phase would not manufacture a screw, Route 2, or a topology, Route 1).
...
- **Screw pitch (`n_z`).** The rotation of the spin-frame about the arrow under transport; *sources* the `γ⁵` coupling (an account-tier identification, not literally the Dirac matrix). Zero by default → vector.
