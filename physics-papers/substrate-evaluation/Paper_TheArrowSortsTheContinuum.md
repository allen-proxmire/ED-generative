...
The decisive move is to stop treating "the continuum limit" as a single step. It is not. No one coarse-grains atoms straight to the equation for how heat spreads; the route is **atoms → a kinetic picture of particles streaming and colliding → the smooth hydrodynamic equations** — *two* coarse-grainings, not one. This layering is textbook kinetic theory. "A layer up" means literally one more coarse-graining applied to the layer below.
...
This has a sharp operator-level form. Compare the heat operator $\partial_t - \nabla^2$ (diffusion, layer-2) with the wave operator $\partial_t^2 - \nabla^2$ (propagation, layer-1, e.g. the coherent field). **They share the same spatial Laplacian.** What differs is the time derivative: paired with $\partial_t^2$ the Laplacian propagates reversibly and keeps the memory; paired with $\partial_t$ it smooths irreversibly and forgets. *Commitment is the order of the time derivative on the shared Laplacian.* Remember-versus-forget, made formal.
...
| Ricci / mean-curvature (gravity) | layer-1 + layer-2 | **measured** (GR-III), inherited |
...
$$ \nabla\!\cdot\!\big(M\,\nabla\varphi\big). $$
So the layer-2 decorrelation is not a family of look-alikes; it is **one operator**, differing only in the field it acts on and the mobility $M$:

- **diffusion** — $M$ constant, $\varphi=\rho$ (measured);
- **viscosity** (Navier–Stokes) — $M=\nu$, $\varphi=u$ (the same velocity decorrelation as diffusion, so the kinematic viscosity has the same substrate origin as the diffusion constant, rather than being merely fitted);
...
The thesis predicts a wall: a law requiring a *reversible* balance asks the substrate to preserve without committing, which a substrate made of commitment cannot do. The sharpest such law is the **soliton** — a localized structure that neither spreads nor decays, dispersion held in perfect balance by a focusing nonlinearity. The prediction is that ED cannot make one. We tested it.
...
This is a **negative result, and it is load-bearing.** A theory that only confirms itself predicts nothing; a theory that names where it must fail, and then fails there on cue, is one that could have been wrong and was not. The soliton edge is the measured coastline that the proven primality edge is the analytic coastline. It also forces a precise statement of the thesis: the native class is *committal structure-making*, not "structure-preserving in general" — reversible preservation is a third thing, neither made nor smoothed, and it is exactly what commitment forbids.
...
- and the laws that need the arrow *absent* — solitons — are **walls**.

One primitive does all of it: it builds (layer 1), it tolerates being forgotten (layer 2, one operator or one constraint), and it forbids (the edge). The same primitive, in the geometric corner, makes horizons and bounds information. The value of the map is not that ED reaches the continuum — a framework that could reach anything would predict nothing — but that it reaches *this* and not *that*, with the boundary drawn by commitment and at least two points on the boundary (primality, solitons) established rather than assumed.

## 10. Conclusion

The continuum laws of physics sort into three classes by a single primitive. Commitment — the arrow, P11 — is native to the laws that keep its footprint (layer-1 structure-making: transport, shocks, the coherent field), reached-with-one-operator for the laws that forget it (layer-2 dissipation: diffusion, viscosity, Ricci-smoothing, with Gaussianity its kernel and capacity its mobility), barred from the laws that need a reversible balance it cannot strike (the soliton edge, measured), and — in the geometric corner of the same measured rule — the maker of horizons. The diffusive results are coefficient-checked; the geometric results are inherited as simulated; the soliton edge is a measured negative; the trophy-problem contributions are physics, with the proofs disclaimed. The arrow sorts the continuum, and it draws the coastline as well as the land.
