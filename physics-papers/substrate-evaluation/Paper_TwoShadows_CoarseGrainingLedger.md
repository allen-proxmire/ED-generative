...
*Scope, stated precisely: the order-erasure mechanism applies to blurs whose kept variables are order-symmetric averages — which is the situation for the geometric and unitary theories below. A blur that keeps fluxes or currents, or that tracks the discard itself, inherits a direction — which is exactly how diffusion (§11) and thermodynamics (§13) retain the arrow the others lost. The ledger's claim is not that every coarse-grained law is reversible; it is that each theory's symmetries are read off its discard column.*
...
1. **The left side (geometry) is itself a double coarse-graining.** The metric is read off graph connectivity, and that takes two steps: the raw hop-count metric is faceted and direction-dependent (layer 1, the ballistic reading); only further averaging produces isotropy (layer 2 — *measured* in the `Paper_MetricFromTheGraph` probes: anisotropy 0.35 → 0.00 as the reach coarsens), while the clean `g ~ 1/b` exponent is *derived from the holographic channel count*, not from transport — its direct layer-2 transport confirmation is that paper's named open item. The smooth Lorentzian manifold sits two floors above the substrate.
...
## 5. Special relativity — keep the cone, blur the frame *(structural ingredients, lens-read)*
...
One entry anchors the whole ledger in arithmetic rather than metaphor, because the blur has been *run*, end to end, on the certified substrate. Coarse-graining the commit-dynamics directly yields a **ballistic kinetic lattice-gas** (layer 1) — certified by build, not asserted. The further blur to the **diffusion PDE** (layer 2) proceeds by decorrelation: **discard velocity memory**, and the artifact is **Markov memorylessness** — the defining "symmetry" of diffusion, its indifference to history. And the decorrelation step is not merely identified-and-pending — it was *tested from the ED side, with a negative result*: the pure substrate **refuses** it (persistence ~0.45, sub-diffusive; the PDE is reached only by leaving ED). Far from undermining the ledger, that supports it: the second blur is the *describer's* forgetting, not the substrate's dynamics — the diffusion equation is a statement about our bookkeeping, and the substrate declines to do our forgetting for us. This section is the existence proof: "take the substrate, blur, obtain the continuum law" is a computation ED has actually performed at least once.

## 12. MOND — where a blur fails *(structural, conditional on P-Quadratic-Strain and the P12-Coh operationalization)*

The ledger also *locates* breakdowns. Newton (§6) discarded the horizon interference term. That discard is *safe* where the local field dominates — and **unsafe below `a₀ ≈ 1.2×10⁻¹⁰ m/s²`, where the discarded term is the larger one** (`a = a_N + √(a_N a_0)`, `Paper_QuadraticStrain`; the constructive sign the cross-term needs — once that paper's one named residual — was subsequently *supplied*, build-verified at MEASURED tier in `Paper_PhaseCoherence_P12Coh` conditional on the P12-Coh operationalization, so it is no longer an open assumption). Galactic rotation curves are not evidence of a new substance; they are **the place where Newton's blur stops being a good approximation** — the un-blurrable residue, surfacing exactly where the corpus result puts it, and just where the ledger's reading expects a discarded column to resurface. MOND phenomenology is a failed coarse-graining diagnosed, not a force invented.
...
**It locates the predictions.** A discarded column never fully vanishes; it *leaks* — though not every leak is measurable. The arrow's leak into gravity (α₁, α₂) is computed and real but suppressed beyond any current experiment (GR-IV). The live tests are the leaks that *are* reachable: the sub-`a₀` regime (Newton's discard resurfacing in galaxies), and the quantum-to-classical boundary at internal multiplicity `M_crit`, with the pre-registered 140–250 kDa matter-wave crossing (the commitment leaking into interferometry). ED's falsifiable content is, almost without exception, **the discarded columns leaking back** — which is exactly where a substrate theory *should* differ from its own shadows.
...
- **The seam:** `Paper_027_Newtons_G.md` (G = c³ℓ_P²/ħ); `Paper_QuadraticStrain_v1.md` (diagonal = Newton, off-diagonal = MOND); `Paper_029_a0.md` (the horizon term); `Paper_PhaseCoherence_P12Coh.md` (the constructive interference sign, supplied at measured tier).
