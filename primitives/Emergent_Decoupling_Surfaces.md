...
> **UPDATE 2026-07-02** (`evaluation/Bits/docs/A2_EmergentBoundary_Bridge_Results.md`, `evaluation/Bits/examples/a2_emergent_boundary_bridge.py`): §7's caveat (b) — "an emergent ρ-wall is soft... would NOT reproduce A1's exactly-zero severance" — has been tested for the bandwidth-collapse route specifically (§4 route 1, the one route this note itself identified as structurally capable of a true cut) and the pessimism does **not** hold there. Bridging the GR-arc's existing dynamical-bandwidth rule (`ḃ=D∇²b−κρ`, already audited elsewhere) into the certified Bits capacity pipeline, with edge bandwidth discretized as the min (not mean) of its two endpoints, produces a dynamically-grown horizon that reproduces A1's exact-zero channel capacity. No contradiction: caveat (b) was really aimed at the soft *state-level* ρ-wall (§4 route 3, already ruled out in this note as a mere potential barrier), not at a genuine structural bandwidth-collapse cut. Caveat (a) (is the collapse law ED-admissible) is not independently re-litigated by this result — it inherits the GR-III rule's existing primitive-consistency audit rather than re-deriving one.
...
Physical horizons (cosmic, black-hole) are by nature **emergent** determinability surfaces — they form from matter/dynamics, not from a hand-set flag. The verdict therefore locates precisely what ED-as-certified lacks for horizons: not a new primitive, but a **state→edge-structure coupling** — a bandwidth-collapse / participation-pruning *dynamics* on the existing P04. This is consistent with the Bits-program reading that the installed boundary is a *toy* surface and that an emergent boundary is "the only path toward anything physical." Two honest caveats, one still open, one now tested: (a) whether such a bandwidth-collapse law is ED-admissible — consistent with the other primitives and crank-safe — is itself still open (not independently re-litigated by the update below, which inherits the GR-III rule's existing audit); (b) ~~an emergent ρ-wall is *soft* (resistance, not exact-zero capacity), so it would **not** reproduce A1's exactly-zero severance~~ **TESTED 2026-07-02, does not hold for the structural (bandwidth-collapse) route** — see the update banner at the top of this note. A *soft state-level* ρ-wall (route 3 above) is still just resistance, not a cut; but a properly-discretized bandwidth-collapse cut (route 1) reproduces A1's exact zero. Naming the missing law was the contribution here; a first build-and-test of it now exists too.
