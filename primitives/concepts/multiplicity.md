...
4. **Multiplicity and entropy.** `S ~ log M_eff` as the ED account of entropy. Formalize and connect to thermodynamic entropy in the thick regime. **Update (2026-07-05):** scoped as a Landauer-bound candidate (P11 commitment is logically an erasure: multi-channel → single-channel, phase content randomized, irreversible). Blocked on two independently-confirmed gaps: (a) no substrate-level temperature exists, and — more than an omission — cannot exist, since Gibbs/thermal-equilibrium symmetry is exactly the two-way symmetry the P11 arrow structurally forbids (`ThreeWalls_OneWay_vs_TwoWay_Continuum.md`); (b) the natural candidate for `M` here (P11's per-commitment channel count) is item 3's still-open, unbounded quantity, not a fixed number. Not closeable without resolving item 3 first, and even then would land at the coarse-grained (thick-regime) tier, not primitive.

5. **Gauge multiplicity.** If gauge structure is channel-topology (Primitive 03 §7, 07 §7), gauge-group dimension is a multiplicity-count of topology classes. Needs explicit treatment. **Update (2026-07-05):** checked directly against P11's commitment-collapse count (item 3/4) — confirmed these are NOT the same object. P11's count is dynamical, chain-local, and transient (a chain's spread across available continuations right before an irreversible collapse); this item's gauge-family count (`Gauge_08_MultiplicityUniqueness_StabilityBound.md`, `Paper_MS-II_MatterSectorFromTheArrow.md` §6) is static and structural (a fixed family size of same-rule-type channels, bounded by internal-amplitude-dimension orthogonality, N≤d). No prior file stated this distinction; it wasn't resolved either way before, just unaddressed. If unifying them is ever attempted, this is the specific gap to close.
